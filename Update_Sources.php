...
$SourceNote = $_POST['note'];

if (isset($_POST['action_button'])){
	$submitted_array = array_keys($_POST['action_button']);
	$CategID=$submitted_array[0];
	
	$SQLCommand = "UPDATE MyMovies.Source SET Name='".$SourceName."',NOTE='".$SourceNote."' WHERE IDSource='".$submitted_array[0]."';";

} Elseif (isset($_POST['delete_button'])){
		$submitted_array = array_keys($_POST['delete_button']);
		$CategID=$submitted_array[0];
		
		$SQLCommand=" Delete FROM MyMovies.Source Where IDSource='".$submitted_array[0]."';";
}

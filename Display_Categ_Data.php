...
$QueryCateg = "select * from Category WHERE IDCategory='".$submitted_array[0]."';";
$Categ = Select_MyMovie($QueryCateg);

while($filme_id = $SQLServerParameterFetch($Categ)) {
	$CategName = $filme_id['Name'];
	$CategNote = $filme_id['Note'];
};

echo '
<form class="form-horizontal" role="form" method="post" action="Update_Categ.php" enctype="multipart/form-data">
';

echo '<div class="form-group">
		<label for="name" class="col-sm-2 control-label">Nome da Classifição</label>
		<div class="col-xs-10 .col-md-6">
			<input type="text" class="form-control" id="name" name="name" placeholder="Nome da Classificação" value="'.$CategName.'">
		</div>
	</div>
';
echo '	<div class="form-group">
		...
		<div class="col-xs-10 .col-md-6">
			<textarea type="memo" class="form-control" id="note" name="note" rows="5" cols="40">'.$CategNote.'</textarea>
		</div>
	</div>
';
if ($_SESSION["Nivel"] == "F"){
